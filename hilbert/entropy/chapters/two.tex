...
the Axioms~\ref{ax:cantor} and~\ref{ax:planck}: every admissible experimental record
...
In this framework, neither is assumed a priori. Both arise as consequences
...
\]
Proper time is the cardinality of this chain. There is no continuum of time
...
operation that enlarges the causal ledger.
...
\NB{Zeno's paradoxes arise precisely from allowing vanishingly small
increments to masquerade as genuine acts.  In the present framework, such
zero-measurement steps are inadmissible.  A measurement must refine, or it is
...
\begin{phenomenon}[The Moment Effect]
The Here and Now is the smooth shadow of the empty open interval between
successive events.  It is not a primitive moment in time, but the continuous
representation of the absence of distinguishable events between
refinements.  Concretely, it appears as the open set
\[
(i,i+1) \subset \mathbb{R},
\]
which contains no admissible events.
\end{phenomenon}
...
“universe” must be constructed as the unique coherent extension of the finite
...
\begin{itemize}
  \item An \emph{event} is a set $e$ such that $e \in E$.
  \item A \emph{worldline} is a set $S$ such that
\[
...
Axiom of Ockham imposes an informational minimality constraint on the
...
\begin{example}[Measurement~\cite{backus1959,naur1963,wheeler1990}]
...
\begin{phenomenon}[The Fourier Budget Constraint]
A refinement ledger possesses a finite informational budget.  To specify a
location with increasing precision consumes refinement capacity that would
otherwise resolve rates of change.  A perfectly sharp position therefore
requires an unbounded refinement cost in momentum.

This is not a probabilistic limitation but a bookkeeping constraint: the
ledger cannot afford to encode arbitrarily precise conjugate refinements
simultaneously.  The apparent uncertainty is the shadow of a finite
refinement budget.
\end{phenomenon}
...
\NB{The restriction operator is the first construction that is intrinsic to the
informational framework developed in this work.  All earlier definitions
(posets, partitions, causal networks, and measurements) are standard
mathematical structures.  The restriction operator encodes admissibility: it
records which refinements survive in the presence of prior observations.  This
mechanism does not appear in classical order theory or in conventional
treatments of causality, and it is the essential ingredient that links local
measurement constraints to global informational consistency.}
...
A refinement $\widehat{R} : \mathbf{i} \to \mathbf{j}$ in $\mathcal{E}$ is
a morphism expressing that $\mathbf{j}$ is the universal solution to a finite
cone of compatibility conditions.  Under the functor $\Phi^{(\bullet)}$, this
...
\Phi^{(m)}(\mathbf{j}).
\]
By functoriality, the square
\[
\begin{tikzcd}
\mathbf{i} @>\widehat{R}>> \mathbf{j} \\
@V\Phi^{(n)}VV @VV\Phi^{(m)}V \\
\Phi^{(n)}(\mathbf{i}) @>\Phi^{(\bullet)}(\widehat{R})>>
\Phi^{(m)}(\mathbf{j})
\end{tikzcd}
\]
commutes.  This is exactly the naturality square.
...
The language of weight captures this asymmetry without importing physical
ontology.  Weight refers solely to the strength with which an event anchors
the admissible encoding of the record.

In later chapters, the smooth shadow of informational weight appears as
inertia and mass.  The terminology is chosen in anticipation of that
correspondence, not derived from it.
